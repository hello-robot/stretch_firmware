...
void write_to_lookup(uint8_t block_id, float * data);
...
int vel_watchdog=CONTROL_RATE_HZ; //Watchdog counts down from 1s
...
void setupHelloController()
{
  ...
  encoder_filter.setupEncoderFilter();
  ...
  trajectory_manager.step();
    ...
    if (dirty_cmd)
    {
     ...
     if (cmd.mode==MODE_VEL_TRAJ || cmd.mode==MODE_VEL_PID)
      ...
      dirty_cmd=0;
      vel_watchdog=CONTROL_RATE_HZ;//1s
