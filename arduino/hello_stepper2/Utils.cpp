...
static const unsigned block_size = 8192; // actual size is 64?
...
{
#ifndef HELLO
  if (0 == (0xFFF & (uintptr_t) blockptr))
  {
    SerialUSB.println();
    SerialUSB.print("0x");
    SerialUSB.print((uintptr_t) blockptr, HEX);
  } else {
    SerialUSB.print(".");
  }
#endif

  flash.erase((const void*) blockptr, sizeof(block));
  flash.write((const void*) blockptr, (const void *) block, sizeof(block));
}

static void store_lookup(float lookupAngle)
{
  block[block_count++] = lookupAngle;
  if(block_count != floats_per_block)
    return;

  // we've filled an entire block, write it to the flash
  write_page();

  // reset our counters and increment our flash block
...
{
  blockptr = (const uint8_t*) lookup + page_id*block_size;
...
void calibrate() {   /// this is the calibration routine
 ...
 // SerialUSB.println(" ");
  ...
  block_count = 0;
  blockptr = (const uint8_t*) lookup;
  SerialUSB.print("Writing to flash 0x");
  SerialUSB.print((uintptr_t) blockptr, HEX);
  SerialUSB.print(" block size PSZ=");
...
void setupTCInterrupts() {  // configure the controller interrupt
  ...
  // Enable GCLK1 48MHz Clock for TC4 (timer counter input clock)
  GCLK->PCHCTRL[TC5_GCLK_ID].reg = (uint16_t) (GCLK_PCHCTRL_CHEN | GCLK_PCHCTRL_GEN_GCLK1);
  GCLK->PCHCTRL[TC4_GCLK_ID].reg = (uint16_t) (GCLK_PCHCTRL_CHEN | GCLK_PCHCTRL_GEN_GCLK1);
  ...
  TC5->COUNT16.CC[0].reg = (int)( round(48000000 / Fs)); //Value to count up to
