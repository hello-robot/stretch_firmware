...
//In contrast to stock Mechaduino code, all actuator specific parameters are zero by default. 
//The paramater data is stored in YAML and required to be pushed down at startup

//Default parameters used in Menu Mode
volatile float Fs = CONTROL_RATE_HZ;   //Sample frequency in Hz

volatile float pKp = 0.0;      //position mode PID values.  Depending on your motor/load/desired performance, you will need to tune these values.  You can also implement your own control scheme
volatile float pKi = 0.0;
volatile float pKd = 0.0;
volatile float pLPF = 0;       //break frequency in hertz


volatile float vKp = 0.0;       //velocity mode PID values.  Depending on your motor/load/desired performance, you will need to tune these values.  You can also implement your own control scheme
volatile float vKi = 0.0;
volatile float vKd = 0.0;
volatile float vLPF = 0.0;       //break frequency in hertz
...
//Encoder calibration.
const float __attribute__((__aligned__(8192))) lookup[16384] = {};
...
//PA default is 1.8 degrees. 
//Adjusting PA (phase advance) sets the max velocity achieved during commutation
//PA of 2.8 = runaway
//PA of 2.5 = fast
//PA of 1.8 = nominal
volatile float PA = 1.8; //aps;            // Phase advance...aps = 1.8 (default) for 200 steps per rev, 0.9 for 400
