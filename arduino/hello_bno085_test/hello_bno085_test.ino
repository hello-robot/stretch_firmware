/*
  ...
  SerialUSB.print it out at 9600 baud to serial monitor.
*/
...
byte imuINTPin = (17u);//3;

void setup()
{
  SerialUSB.begin(115200);
  SerialUSB.println();
  SerialUSB.println("BNO080 Read example with Interrupt handler, getReading and multiple access");
...
//  //=================================
  ...
  {
    SerialUSB.println("BNO080 not detected at default I2C address. Check your jumpers and the hookup guide. Freezing...");
    while (1);
  }
  ...
  SerialUSB.println(F("LinearAccelerometer enabled, Output in form x, y, z, accuracy, in m/s^2"));
  SerialUSB.println(F("Rotation vector, Output in form i, j, k, real, accuracy"));
...
void loop()
{
  //SerialUSB.println("Doing other things");
  ...
  if(newLinAcc) {
    ...
    SerialUSB.print(ax, 2);
    SerialUSB.print(F(","));
    SerialUSB.print(ay, 2);
    SerialUSB.print(F(","));
    SerialUSB.print(az, 2);
    SerialUSB.print(F(","));
    SerialUSB.print(az, 2);
    SerialUSB.print(F(","));
...
{
  if (accuracyNumber == 0) SerialUSB.println(F("Unreliable"));
  else if (accuracyNumber == 1) SerialUSB.println(F("Low"));
  else if (accuracyNumber == 2) SerialUSB.println(F("Medium"));
