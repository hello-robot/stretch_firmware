...
void TraceManager::update_trace_status(Wacc_Status * stat)
{
  if(trace_on && TRACE_TYPE==TRACE_TYPE_STATUS)
  {
    memcpy((uint8_t *)(raw_data+trace_write_idx*sizeof(Wacc_Status)) ,(uint8_t *)(stat) ,sizeof(Wacc_Status));
...
int TraceManager::rpc_read(uint8_t * rpc_out)
{
  int num_byte_rpc_out=0;
      ...
      if(TRACE_TYPE==TRACE_TYPE_STATUS)
      {
          memcpy(rpc_out + 3, (uint8_t *)&(raw_data[trace_read_idx*sizeof(Wacc_Status)]), sizeof(Wacc_Status)); //Collect the status data
          num_byte_rpc_out=sizeof(Wacc_Status)+3;
          trace_read_idx++;
