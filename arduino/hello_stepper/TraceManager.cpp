...
void TraceManager::update_trace_status(Status * stat)
{
  if(trace_on && TRACE_TYPE==TRACE_TYPE_STATUS)
  {
    memcpy((uint8_t *)(raw_data+trace_write_idx*sizeof(Status)) ,(uint8_t *)(stat) ,sizeof(Status));
...
int TraceManager::rpc_read(uint8_t * rpc_out)
{
  int num_byte_rpc_out=0;
      ...
      if(TRACE_TYPE==TRACE_TYPE_STATUS)
      {
          memcpy(rpc_out + 3, (uint8_t *)&(raw_data[trace_read_idx*sizeof(Status)]), sizeof(Status)); //Collect the status data
          num_byte_rpc_out=sizeof(Status)+3;
          trace_read_idx++;
