...
class RunstopManager {    
  public: 
    RunstopManager()
    {
      ...
      alert_trigger_runstop=false;          //External even triggered runstop (RPC, low volage)
    ...
    void activate_runstop() //Called from TC4 ISR
    {
      alert_trigger_runstop=1;
    }

    void deactivate_runstop() //Called from TC4 ISR
    {
      runstop_mode=MODE_RUNSTOP_NOT_ACTIVE;
      state_runstop_event=0;
    }

    void step() //Called at 100hz from TC4 loop
    {
...
void stepPimuController()
{
  ...
  if (dirty_trigger)
  {
    ...
    if (trg.data & TRIGGER_CLOCK_ZERO)
    {
         time_manager.clock_zero();
    }
    if (trg.data & TRIGGER_STATUS_SYNC)
    {
         sync_manager.trigger_status_sync();
    }
    dirty_trigger=false;
